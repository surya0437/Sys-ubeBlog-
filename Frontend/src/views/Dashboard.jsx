...
export default function Dashboard() {
    ...
    useEffect(() => {
        async function fetchBlogs() {
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/getBlog');
    ...
    return (
        <>
            <h4 className="mb-4 text-3xl font-semibold text-gray-600 dark:text-gray-300">
                Blog List
            </h4>
                    ...
                    <table className="w-full whitespace-no-wrap">
                        ...
                        <tbody className="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            {blogs.map((blog, index) => (
                                <tr key={blog.id} className="text-gray-700 dark:text-gray-400">
                                    <td className="px-4 py-3">
                                        {index + 1}
                                    </td>
                                    <td className="py-3">
                                        <img className="object-cover w-16" src={blog.blogImage} alt={blog.title} loading="lazy" />
                                    </td>
                                    <td className="px-4 py-3">
                                        {blog.title}
                                    </td>
                                    ...
                                    <td className="px-4 py-3 text-xs">
                                        <span className={`px-2 py-1 font-semibold leading-tight rounded-full ${blog.status ? 'text-green-700 bg-green-100' : 'text-red-700 bg-red-100'}`}>
